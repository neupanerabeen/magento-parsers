...
class RelatedProducts:
    ...
    def transform(self):
        ...
        for key,value in configDict.items():
            try:
                ...
                pass
        self.output=self.output[self.output["related_skus"].notnull()]

    def modify(self):
    	self.output.apply(lambda x: self.getSku(x), axis=1)
        self.output["related_skus"]=self.image_list
        del self.output["id"]
        del self.output["crosssell_skus"]
    
    def getSku(self,row):
    	sku=row["sku"]
        id=row["id"]
        images=row["related_skus"].split(" ")
        sk=""
    ...
    def export(self):
        ...
        self.output.to_csv("output/relatedProducts.csv", index = False)
